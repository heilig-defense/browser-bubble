...
//		npm i websocket
//
//	This example shows how Browser Bubble Pro can be used with Playwright.
//
...
//	has been added to the browser list.  The correct connect command for Playwright 
//	is .connectOverCDP().
// 
//	For this example, we first add the Chromium browser that was installed with 
//	Playwright.  The path for chrome.exe will be deep in the npm directory generally
//	under the c:\users\<username>\appdata\roaming\ directory.  The Playwright docs
...
client.on('connect', function(connection) {
							...
							if (action['SubAction'] == BB_SERVER_BUBBLE_SUB_ACTION_READY) {
								...
								(async () => {
									const browser = await chromium.connectOverCDP(
										'ws://localhost:' + dbg['Port'] + '/devtools/browser/' + dbg['WsEndPoint']
									);
									
									const page = await browser.newPage();
									await page.goto('https://abrahamjuliot.github.io/creepjs');
									//Give CreepJS enough time to load...
	...
	function addBrowser() {
		...
		var ba = {};
		ba['ID'] = 0;
		ba['Name'] = "Chromium (Playwright)";
		ba['BrowserType'] = BB_BROWSER_CHROMIUM;
		ba['FilePath'] = "c:\\users\\cydec\\appdata\\local\\ms-playwright\\chromium-956323\\chrome-win\\chrome.exe";
